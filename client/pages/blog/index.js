...
async function getPosts(page=1) {
  ...
  const res = await fetch(`${process.env.NEXT_PUBLIC_HOST}/api/post?${query}`)
...
function Blog(props) {
  ...
  async function loadMorePosts() {
    const newData = await getPosts(posts.page+1);
    const newPosts = [...posts.docs, ...newData.docs];
    newData.docs = newPosts;
    setPosts(newData);
  }
                            ...
                            {posts.hasNextPage && (
                                <div className="row">
                                    <div className="col-lg-12 text-center mt-20 mb-30">
                                        <a onClick={loadMorePosts} className="theme_btn">Load More</a>
                                    </div>
                                </div>
                            )}
                        </div>
